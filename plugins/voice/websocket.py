"""RFC 6455, vendored from the Discord plugin.

The same file, copied rather than shared, because a plugin is a directory the host copies and
starts on its own — two plugins cannot import from one another. It is worth the duplication: this
client has performed a real handshake against a real service and been disconnected for masking a
frame wrongly, which is more than a freshly written one could claim.
